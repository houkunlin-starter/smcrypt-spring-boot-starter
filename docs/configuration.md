...
| 配置项              | 说明                                                                                    | 默认值  |
|---------------------|-----------------------------------------------------------------------------------------|---------|
| `smcrypt.fail-fast` | 解密失败（含未找到密钥）时是否中断启动：`true` 抛出异常终止启动，`false` 记录日志并跳过 | `false` |
| `smcrypt.strict`    | 加密配置安全体检：`off` 不体检 / `warn` 仅告警 / `fail` 存在问题时中断启动              | `off`   |

> 默认行为是「单个属性解密失败只记录日志并跳过」，可能导致应用带着未解密的密文继续启动。
> 对配置正确性要求较高的生产环境，建议设置 `smcrypt.fail-fast=true`，让配置问题在启动阶段即暴露。
> `smcrypt.strict` 另兼容 `true`（等同 `fail`）与 `false`（等同 `off`），体检规则见
> [生产环境加固清单](security.md#生产环境加固清单)。
